...
function indekseGoreFenomen(paramDizi, paramIndex) {
  const secilenFenomen = paramDizi[paramIndex];
  console.log("indexGöre", secilenFenomen);
  // return paramIndex + ". indekste bulunan fenomen: " + secilenFenomen.profile;
  return `${paramIndex}. indekste bulunan fenomen: ${secilenFenomen.profile}`;
}
...
function profilListesi(fenomen) {
  ...
  for (let i = 0; i < fenomen.length; i++) {
    yeniListe.push(fenomen[i]);
  }

  for (let i = 0; i < yeniListe.length; i++) {
    yeniListe[i] = yeniListe[i].profile;
  }
...
function fenomenSil(fenomen, sil) {
  const yeniListe = [...fenomen];

  yeniListe.splice(sil, 1);
...
function fenomenEkle(newarray, numara, profil, takipçi, gönderiler, medya) {
  const yeniListe = [...newarray];
  const item = {
    number: numara,
    profile: profil,
    followers: takipçi,
...
function enFenomenler(Fenomen) {
  const yeniListe = []; // Kopyalamanın 2. yolu ES6

  for (let i = 0; i < Fenomen.length; i++) {
    if (Fenomen[i].followers > 100000000) {
      yeniListe.push(Fenomen[i].profile);
    }
  }
...
function fenomenGonderimSayisi(fenomen, fenomenProfil) {
  let gönderim = 0;

  for (let i = 0; i < fenomen.length; i++) {
    if (fenomen[i].profile === fenomenProfil) {
      gönderim = fenomen[i].posts;
    }
  }
...
function platformaGoreCokGonderiYapanFenomen(fenomen, fenomenProfil) {
  let encokgonderiadi = "";
